    ...
    function verify(code) {
        ...
        .then(data => {
            ...
            // Di dalam fungsi verify(code) -> .then(data => { ... })

    if (data.success) {
        if (navigator.vibrate) navigator.vibrate([120, 60, 120]);
        
        // Tentukan warna berdasarkan aksi
        const isSuccess = data.action === 'keluar' || data.action === 'kembali';
        ...
        if (data.action === 'keluar' && data.is_sampai_pulang) {
            extraInfo = `<div class="mt-2 p-2 bg-amber-100 border border-amber-200 rounded-lg text-[11px] text-amber-800 font-semibold">
                            <i class="fas fa-info-circle mr-1"></i> Dispensasi sampai pulang. Tidak wajib scan kembali.
                        </div>`;
        } else if (data.is_terlambat) {
            extraInfo = `<div class="mt-2 p-2 bg-red-100 border border-red-200 rounded-lg text-[11px] text-red-800 font-semibold">
                            <i class="fas fa-exclamation-triangle mr-1"></i> Siswa terlambat kembali!
                        </div>`;
        }

        result.innerHTML = `
            <div class="flex items-center space-x-3 mb-3">
                <div class="w-11 h-11 rounded-full bg-${themeColor}-500 text-white flex items-center justify-center text-lg shadow-lg">
                    <i class="fas ${icon}"></i>
                </div>
                <div>
