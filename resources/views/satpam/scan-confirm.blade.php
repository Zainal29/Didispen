...
@section('content')
<div class="max-w-2xl mx-auto space-y-4 w-full min-w-0">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-5">
        ...
        {{-- Data Dispensasi --}}
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 mb-4 space-y-3">
            <div>
                <p class="text-[10px] font-bold text-blue-600 uppercase">Nomor Surat</p>
                <p class="font-mono font-semibold text-gray-900 break-all">{{ $dispensasi->nomor_surat }}</p>
            </div>
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 sm:p-4 mb-4">
            ...
            <div class="space-y-2 text-xs sm:text-sm">
                <div class="flex justify-between gap-2">
                    <span class="text-gray-500 flex-shrink-0">Nama Lengkap:</span>
                    <span class="font-semibold text-gray-900 text-right break-words">{{ $dispensasi->siswa->nama_lengkap }}</span>
                </div>
                <div class="flex justify-between gap-2">
                    <span class="text-gray-500 flex-shrink-0">NIS/NISN:</span>
                    <span class="font-mono font-semibold text-gray-900 text-right break-all">{{ $dispensasi->siswa->user->nis_nip ?? '-' }}</span>
                </div>
                <div class="flex justify-between gap-2">
                    <span class="text-gray-500 flex-shrink-0">Kelas:</span>
                    <span class="font-semibold text-gray-900 text-right">{{ $dispensasi->siswa->kelas->nama_kelas ?? '-' }}</span>
                </div>
                <div class="flex justify-between gap-2">
                    <span class="text-gray-500 flex-shrink-0">Jurusan:</span>
                    <span class="text-gray-700 text-right break-words">{{ $dispensasi->siswa->kelas->jurusan->nama_jurusan ?? '-' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-4">
            <div class="mb-2">
                ...
                <p class="text-xs sm:text-sm font-semibold text-gray-900 break-words">{{ $dispensasi->tujuan }}</p>
            </div>
            <div>
                <p class="text-[10px] font-bold text-gray-500 uppercase">Alasan</p>
                <p class="text-xs sm:text-sm text-gray-700 break-words">{{ $dispensasi->alasan }}</p>
            </div>
        </div>
        ...
        <form action="{{ route('satpam.scan.verify') }}" method="POST" class="space-y-3">
            ...
            <button type="submit" class="w-full py-3 min-h-[44px] bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors flex items-center justify-center text-sm">
        ...
        <form action="{{ route('satpam.scan.verify') }}" method="POST" class="space-y-3">
            ...
            <button type="submit" class="w-full py-3 min-h-[44px] bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg transition-colors flex items-center justify-center text-sm">
        ...
        {{-- Tombol Kembali --}}
        <a href="{{ route('satpam.scan') }}" class="inline-flex items-center justify-center w-full min-h-[44px] mt-3 text-gray-600 hover:text-gray-800 font-semibold text-sm">
            <i class="fas fa-redo mr-1.5"></i>Scan QR Code Lainnya
        </a>
    </div>
</div>
@endsection
